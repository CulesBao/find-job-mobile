...
class _EditJobPageState extends State<EditJobPage> {
  ...
  void _initializeForm() {
    ...
    _responsibilityController.text = job.responsibility ?? '';
    _selectedJobType = job.jobType;
    _selectedEducation = _parseEducation(job.education);
    _selectedSalaryType = job.salaryType ?? SalaryType.monthly;
    _selectedCurrency = job.currency ?? Currency.vnd;
    ...
    if (job.expiredAt != null) {
      try {
        _expiredDate = DateTime.parse(job.expiredAt!);
      } catch (e) {
        // ignore invalid date
      }
    }
  ...
  Education? _parseEducation(String? educationStr) {
    if (educationStr == null) return null;
    
    // Map from API string values to enum
    switch (educationStr.toUpperCase()) {
      case 'HIGH_SCHOOL':
  ...
  Future<void> _handleUpdateJob() async {
    ...
    try {
      ...
      final response = await _jobRepository.updateJobById(widget.job.id, request);
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: _isLoading ? null : _handleUpdateJob,
